...
public partial class Player : CharacterBody2D
{
	...
	{
		// Tot: kein Input, keine Bewegung — State unverändert zurückgeben
		if (_moveState == MoveState.Dead)
			return new PlayerState { Position = Position, Velocity = Vector2.Zero };

		// 1. Einmalige Aktionen (Angriff/Rolle) — nur starten wenn nichts anderes läuft
		...
		// 4. Resultierenden State zurückgeben (für Server-Reconciliation)
	...
	private void UpdateAnimation(string dirName)
	{
		...
		UpdateWeaponZIndex(dirName); // Waffe vor Spieler wenn nach rechts oder unten läuft, sonst dahinter
	}
