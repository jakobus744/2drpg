...
public partial class InventorySlotUI : Control
{
    ...
    // Design-Zeit-Identität für im Editor platzierte Slots:
    // - Equipment-Slot: DesignEquipSlot setzen (Weapon/Offhand/...)
    // - Hotbar/Grid-Slot: DesignType + DesignIndex setzen
    [Export] public EquipSlot DesignEquipSlot = EquipSlot.None;
    [Export] public SlotType DesignType = SlotType.Hotbar;
    [Export] public int DesignIndex = 0;

    // Adresse aus den Design-Feldern (für editor-platzierte Slots)
    public SlotAddress ResolveDesignAddress()
    {
        if (DesignEquipSlot != EquipSlot.None)
            return SlotAddress.Equipment(DesignEquipSlot);
        return DesignType == SlotType.Grid
            ? SlotAddress.Grid(DesignIndex)
            : SlotAddress.Hotbar(DesignIndex);
    }
    ...
    public override bool _CanDropData(Vector2 atPosition, Variant data)
    {
        ...
        GD.Print($"[Drop] ziel={Address.Type}/{Address.Equip} item={src?.GetStack()?.Data?.ItemId ?? "-"} itemSlot={src?.GetStack()?.Data?.Slot}");
